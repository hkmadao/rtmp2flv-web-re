...
const MainTableLayout: FC = () => {
  ...
  const handleSelect = (
    ...
    if (rowSelectionType == 'checkbox') {
      const newKeys = selectedRows.map(r => r.idClientInfo!);
      dispatch(actions.setSelectedRowKeys(newKeys));
    } else {
      dispatch(actions.setSelectedRowKeys([record.idClientInfo!]));
    }
  }

  const onRow = (record: TClientInfo) => {
    return {
      onClick: (event: any) => {
        if (rowSelectionType == 'checkbox') {
          let newKeys = tableStore.selectedRowKeys?.slice() || [];
          if (!newKeys.includes(record.idClientInfo!)) {
            newKeys?.push(record.idClientInfo!);
            dispatch(actions.setSelectedRowKeys(newKeys));
          } else {
            newKeys = newKeys?.filter(
              (idKey) => idKey !== record.idClientInfo,
            );
            dispatch(actions.setSelectedRowKeys(newKeys));
          }
